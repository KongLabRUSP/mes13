...
dt1 <- fread("data/Renyi_12292017/combined_WJ_anno.csv")
...
# Ratios----
dt.pr$`diff(hg,lg)` <- dt.pr$HG - dt.pr$LG
dt.pr$`mean(hg,lg)` <- (dt.pr$HG + dt.pr$LG)/2
...
tiff(filename = "tmp/diff_vs_mean_hg_lg.tiff",
     height = 5,
     width = 5,
     units = 'in',
     res = 300,
     compression = "lzw+p")
plot(dt.pr$`diff(hg,lg)` ~ dt.pr$`mean(hg,lg)`,
     xlab = "Mean(HG,LG)",
     ylab = "Diff(HG,LG)")
abline(h = c(-20, 20),
       lty = 2)
...
# Scale and center
dt.pr$scale.hg.lg <- scale(dt.pr$`diff(hg,lg)`)

hist(dt.pr$`diff(hg,lg)`, 100)
hist(dt.pr$scale.hg.lg, 100)
hist(abs(dt.pr$scale.hg.lg), 100)

# Rank genes by the absolute differences,
# plot top20 genes
tmp <- subset(dt.pr,
              !is.nan(scale.hg.lg))
tmp <- tmp[order(abs(tmp$scale.hg.lg),
                 decreasing = TRUE), ]
tmp <- tmp[1:20, ]
tmp
...
p2 <- ggplot(data = dtl) +
  ...
  scale_y_discrete("Gene",
                   expand = c(0, 0)) +
  ggtitle("MES13: Top 20 Genes\nWith Largest Differences in HG vs. LG Promoters") +
  theme(axis.text.x = element_text(angle = 30,
                                   hjust = 1),
        plot.title = element_text(hjust = 0.5))
p2

tiff(filename = "tmp/top20_hg.vs.lg_all.tiff",
     height = 6,
     width = 6,
     units = 'in',
     res = 300,
...
tiff(filename = "tmp/top20_hg.vs.lg_diff.tiff",
     height = 6,
     width = 6,
     units = 'in',
     res = 300,
...
# Largest change between HG vs. LG and MIC vs. HG----
dt.pr$hg.lg_mic.hg <- dt.pr$`diff(hg,lg)` - dt.pr$`diff(mic,hg)`
tmp <- subset(dt.pr,
              !is.nan(hg.lg_mic.hg))
tmp <- tmp[order(abs(tmp$hg.lg_mic.hg),
                 decreasing = TRUE), ]
tmp <- tmp[1:40, ]
# Order by HG vs. LG
...
tiff(filename = "tmp/top40_hg.vs.lg_mic.vs.hg.tiff",
     height = 8,
     width = 5,
     units = 'in',
     res = 300,
