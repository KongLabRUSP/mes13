---
title: "DNA methylome and transcriptome alterations in a high-glucose induced diabetic nephropathy cellular model and identification of novel targets for treatment by FX"
output:
  html_notebook:
    number_sections: yes
    toc: TRUE
...
Rasika Hudlikar^c^, Wenji Li^a,b,c^, Davit Sargsyan^c,d,e^, Renyi Wu^c,d^, Shanyi Li^c,d^, Lujing Wang^c,d,e^, Ah-Ng Kong^c,d^  
...
trt.names <- c("LG",
               "HG",
               "MIC_1.5uM",
               "TIIA_5uM", # NOTE: corrected from "FX_1uM" on 05/25/2020!
               "FX_1uM",
               "Gen_10uM",
               "Ber_6uM")
...
## Keep only LG, HG and FX samples
```{r samples}
dt1 <- droplevels(dt1[, c("GeneNames",
                          "LG",
                          "HG",
                          "FX_1uM")])
datatable(head(dt1))
...
### FX - HG
```{r degseq_fx_hg, fig.height = 6, fig.width = 6}
DEGexp(
  geneExpMatrix1 = dt1,
  geneCol1 = which(colnames(dt1) == "GeneNames"),
  expCol1 = which(colnames(dt1) == "FX_1uM"),
  groupLabel1 = "FX_1uM",
...
fx_hg <- fread("tmp/fx_hg/output_score.txt")
fx_hg

# Write as CSV----
write.csv(fx_hg,
          file = "tmp/fx_hg/MES13_RNA_DEGseq_FX-HG.csv",
          row.names = FALSE)
...
fx_hg <- merge(fx_hg,
               dt1.fpm,
               by = "GeneNames")
fx_hg[, mu := (log2(FX_1uM_fpm + 1) + log2(HG_fpm + 1)) / 2]
fx_hg[, diff := log2(FX_1uM_fpm + 1) - log2(HG_fpm + 1)]

tiff(
  filename = "tmp/fx_hg/fig1b_mes13_fx_hg_maplot.tiff",
  height = 6,
  width = 6,
  units = 'in',
  res = 600,
...
points(fx_hg$diff[fx_hg$`q-value(Storey et al. 2003)` < 0.5 &
                    fx_hg$diff > 0] ~ fx_hg$mu[fx_hg$`q-value(Storey et al. 2003)` < 0.5 &
                                                 fx_hg$diff > 0] ,
       pch = "x",
       col = "green")
points(fx_hg$diff[fx_hg$`q-value(Storey et al. 2003)` < 0.5 &
...
## FIGURE 3: combine th etwo MA plots
```{r ma_plot, fig.height = 4, fig.width = 7}
dt.ma <- rbindlist(list(data.table(comp = "HG - LG",
                                   mu = hg_lg$mu,
                                   diff = hg_lg$diff,
                                   up = hg_lg$`q-value(Storey et al. 2003)` < 0.5 & hg_lg$diff > 0,
                                   dn = hg_lg$`q-value(Storey et al. 2003)` < 0.5 & hg_lg$diff < 0),
                        data.table(comp = "FX - HG",
                                   mu = fx_hg$mu,
                                   diff = fx_hg$diff,
                                   up = fx_hg$`q-value(Storey et al. 2003)` < 0.5 & fx_hg$diff > 0,
                                   dn = fx_hg$`q-value(Storey et al. 2003)` < 0.5 & fx_hg$diff < 0)))
dt.ma$comp <- factor(dt.ma$comp,
                     levels = c("HG - LG",
                                "FX - HG"))
dt.ma$col <- 1
dt.ma$col[dt.ma$up] <- 2
dt.ma$col[dt.ma$dn] <- 3
dt.ma$col <- factor(dt.ma$col)
...
p1 <- ggplot(dt.ma,
  ...
  geom_hline(yintercept = c(-0.3, 0.3),
             linetype = "dashed") +
  scale_x_continuous("Mean Log2(FPM + 1)") +
  scale_y_continuous("Difference Log2(FPM + 1)") +
...
tiff(filename = "tmp/fig3_mes13_fx_hg_maplot.tiff",
     height = 4,
     width = 7,
     units = 'in',
     res = 600,
...
g2 <- hg_lg[`q-value(Storey et al. 2003)` < 0.5 & 
              `log2(Fold_change) normalized` < -0.3,]$GeneNames
g3 <- fx_hg[`q-value(Storey et al. 2003)` < 0.5 & 
              `log2(Fold_change) normalized` > 0.3,]$GeneNames
g4 <- fx_hg[`q-value(Storey et al. 2003)` < 0.5 & 
              `log2(Fold_change) normalized` < -0.3,]$GeneNames
...
p1 <- ggplot() +
  geom_circle(aes(x0 = c(1, 2, 1, 2),
                  ...
                  fill = factor(c(2, 1, 1, 2))),
              size = 2,
              alpha = 0.3) +
  geom_text(aes(x = rep(c(0.5, 1.5, 2.5), 2),
                y = rep(c(1.1, 4.1), each = 3),
                label = format(c(length(g2) - length(dn.up), 
                                 length(dn.up), 
                                 length(g3) - length(dn.up),
                                 length(g1) - length(up.dn), 
                                 length(up.dn), 
                                 length(g4) - length(up.dn)),
                               big.mark = ",")),
            size = 7) +
  geom_text(aes(x = c(1, 2, 1, 2),
                y = c(2.3, 2.3, 5.3, 5.3),
                # color = factor(c(2, 1, 1, 2)),
                label = c(paste0("HG - LG\n(", length(g2), ")"),
                          paste0("FX - HG\n(", length(g3), ")"),
                          paste0("HG - LG\n(", length(g1), ")"),
                          paste0("FX - HG\n(", length(g4), ")"))),
            size = 7) +
  geom_text(aes(x = c(3.25, 3.25),
                y = c(1.1, 4.1),
                label = c(length(g2) + length(g3) - length(dn.up),
                          length(g1) + length(g4) - length(up.dn))),
            size = 7) +
...
write.csv(t1,
          file = "tmp/mes13_fx_rnaseq_degseq_genes_q-0.5_log2-0.3.csv",
          row.names = FALSE)
...
## FIGURE 4: Heatmap and Venn Diagram
```{r heatmap-venn, fig.height = 10, fig.width = 18}
# # Blank canvas
# p3 <- ggplot() + theme_void()

tiff(filename = "tmp/fig4_mes13_fx_hitmap_with_phylo_and_venn.tiff",
     height = 10,
     width = 18,
     units = 'in',
     res = 600,
...
gene_names <- unique(c(g1, g2, g3, g4))
length(gene_names)
...
gene_map <- rbindlist(entraz_id)

# Pathway Enrichment Analysis of a gene set. 
# Given a vector of genes, this function will return 
# the enriched pathways with FDR control.
# a. log2(HG - LG) > 0.3
m1 <- enrichPathway(gene = gene_map$entraz_id[gene_map$gene %in% g1],
                    pvalueCutoff = 0.5, 
                    readable = TRUE, 
                    organism = "mouse")
t1 <- as.data.table(m1)
...
# b. log2(HG - LG) < -0.3
m2 <- enrichPathway(gene = gene_map$entraz_id[gene_map$gene %in% g2],
                    pvalueCutoff = 0.5, 
                    readable = TRUE, 
                    organism = "mouse")
t2 <- as.data.table(m2)
...
# c. log2(FX - HG) > 0.3
m3 <- enrichPathway(gene = gene_map$entraz_id[gene_map$gene %in% g3],
                    pvalueCutoff = 0.5, 
                    readable = TRUE, 
                    organism = "mouse")
t3 <- as.data.table(m3)
...
# d. log2(FX - HG)< -0.3
m4 <- enrichPathway(gene = gene_map$entraz_id[gene_map$gene %in% g4],
                    pvalueCutoff = 0.5, 
                    readable = TRUE, 
                    organism = "mouse")
t4 <- as.data.table(m4)
...
# e. Up-Down: log2(HG - LG) > 0.3 & log2(FX - HG) < -0.3
m5 <- enrichPathway(gene = gene_map$entraz_id[gene_map$gene %in% up.dn],
                    pvalueCutoff = 0.5, 
                    readable = TRUE, 
                    organism = "mouse")
t5 <- as.data.table(m5)
...
# f. Down-Up: log2(HG - LG) < -0.3 & log2(FX - HG) > 0.3
m6 <- enrichPathway(gene = gene_map$entraz_id[gene_map$gene %in% dn.up],
                    pvalueCutoff = 0.5, 
                    readable = TRUE, 
                    organism = "mouse")
t6 <- as.data.table(m6)
...
```{r reactome_fig_tab, fig.height = 5, fig.width = 10}
barplot(m1, 
        showCategory = 20,
        title = "log2(HG - LG) > 0.3")
...
barplot(m5, 
        showCategory = 20,
        title = "log2(HG - LG) > 0.3 & log2(FX - HG)< -0.3")

tiff(filename = "tmp/mes13_fx_reactome_up_down.tiff",
     height = 5,
     width = 10,
     units = 'in',
     res = 600,
...
barplot(m6, 
        showCategory = 20,
        title = "log2(HG - LG) > 0.3 & log2(FX - HG)< -0.3")
tiff(filename = "tmp/mes13_fx_reactome_down_up.tiff",
     height = 5,
     width = 10,
     units = 'in',
     res = 600,
...
# Save CSV files
write.csv(t1, file = "tmp/a.csv")
write.csv(t2, file = "tmp/b.csv")
write.csv(t3, file = "tmp/c.csv")
write.csv(t4, file = "tmp/d.csv")
write.csv(t5, file = "tmp/e.csv")
write.csv(t6, file = "tmp/f.csv")
```
...
```{r avg_methyl, fig.height = 6, fig.width = 5}
mumth <- list()
for (i in 1:ncol(pct)) {
...
pctMeth$`FX-HG DNA` <- 100*(pctMeth$WJ05 - pctMeth$WJ02)
...
```{r rna_diff}
expRNA <- fread("data/mes13_fx_rnaseq_degseq_genes_q-0.5_log2-0.3.csv")
...
g3 <- rna_dna[rna_dna$`FX-HG DNA` >= 10 & 
                rna_dna$`FX-HG` <= -0.3 &
                reg == "Promoter"]
length(unique(g3$gene))

g4 <- rna_dna[rna_dna$`FX-HG DNA` <= -10 & 
                rna_dna$`FX-HG` >= 0.3 &
                reg == "Promoter"]
...
write.csv(g2,
          ...
          row.names = FALSE)
write.csv(g3,
          file = "tmp/dna.up_rna.dn_fx-hg.csv",
          row.names = FALSE)
write.csv(g4,
          file = "tmp/dna.dn_rna.up_fx-hg.csv",
          row.names = FALSE)
```
...
```{r methyl_heatmap_no_5p_3p, fig.height = 6, fig.width = 5}
...
tiff(filename = "tmp/fig7_mes13_gene_methyl_no_5p_3p.tiff",
     height = 6,
     width = 5,
     units = 'in',
     res = 600,
...
## HG vs. LG Starburst
```{r starburst_hg_lg, fig.height = 5, fig.width = 7}
tmp <- rna_dna[gene %in% unique(c(g1$gene,
                                  g2$gene)), ]
tmp$reg <- factor(tmp$reg,
                  levels = rev(levels(tmp$reg)))

tmp$a <- 0.7
tmp$a[tmp$`HG-LG DNA` > -10 & 
        tmp$`HG-LG DNA` < 10] <- 0.3
...
tmp1 <- unique(tmp[gene %in% unique(g1$gene) &
                     reg == "Promoter",
                   c("gene",
                     "reg",
                     "HG-LG")])
setkey(tmp1,
       `HG-LG`)
tmp1$ypos <- seq(from = min(tmp$`HG-LG`), 
                 to = max(tmp$`HG-LG`), 
...
setkey(tmp2,
       `HG-LG`)
tmp2$ypos <- seq(from = min(tmp$`HG-LG`), 
                 to = max(tmp$`HG-LG`), 
...
p1 <- ggplot(data = tmp,
  ...
  geom_vline(xintercept = c(-10, 10),
             linetype = "dashed") +
  geom_point(data = tmp[reg != "Promoter",],
             aes(alpha = a),
             size = 2,
             shape = 21) +
  geom_point(data = tmp[reg == "Promoter",],
...
tiff(filename = "tmp/fig8a_starburst_hg-lg.tiff",
     height = 5,
     width = 7,
     units = 'in',
     res = 600,
...
## FX vs. HG Starburst
```{r starburst_fx_hg, fig.height = 5, fig.width = 7}
tmp <- rna_dna[gene %in% unique(c(g3$gene,
                                  g4$gene)), ]
tmp$reg <- factor(tmp$reg,
                  levels = rev(levels(tmp$reg)))

tmp$a <- 0.7
tmp$a[tmp$`FX-HG DNA` > -10 & 
        tmp$`FX-HG DNA` < 10] <- 0.3
...
tmp3 <- unique(tmp[gene %in% unique(g3$gene) &
                     reg == "Promoter",
                   c("gene",
                     "reg",
                     "FX-HG")])
setkey(tmp3,
       `FX-HG`)
tmp3$ypos <- seq(from = min(tmp$`FX-HG`), 
                 to = max(tmp$`FX-HG`), 
...
setkey(tmp4,
       `FX-HG`)
tmp4$ypos <- seq(from = min(tmp$`FX-HG`), 
                 to = max(tmp$`FX-HG`), 
...
tiff(filename = "tmp/fig8b_starburst_fx-hg.tiff",
     height = 5,
     width = 7,
     units = 'in',
     res = 600,
...
## Lollipop plo: SELECT GENES!
```{r lollipop, fig.height = 7, fig.width = 12}
# Genes with RNA down/DNA up----
gene.keep <- unique(c(tmp2$gene[tmp2$gene %in% tmp3$gene],
                      tmp1$gene[tmp1$gene %in% tmp4$gene]))
...
dna$`FX-HG` <- 100*(dna$WJ05 - dna$WJ02)
